    ...
    <section className="py-[var(--section-gap)] bg-[color:var(--surface)]">
      <Container>
        <div className="flex items-end justify-between">
          ...
          <Link
            href={`/${locale}/blog`}
            className="hidden text-sm font-medium text-[#ce0e2d] hover:underline sm:block"
          >
            전체보기 →
          </Link>
        </div>
          ...
          {posts.map((post) => {
            ...
            return (
              <Link
                ...
                className="group rounded-2xl border border-[color:var(--border)] bg-white p-6 transition-all hover:border-[#ce0e2d]/30 hover:shadow-[var(--shadow-medium)]"
              >
                {/* 카테고리 */}
                <span className="inline-block rounded-full bg-[#ce0e2d]/10 px-3 py-1 text-xs font-medium text-[#ce0e2d]">
                  {category?.name || post.category}
                </span>
                {/* 제목 */}
                <h3 className="mt-4 text-lg font-bold text-[color:var(--fg)] transition-colors group-hover:text-[#ce0e2d] line-clamp-2">
                  {post.title}
                </h3>
                {/* 요약 */}
